...
## Forces
The forces involved in the External-Call Pattern are extensibility and reusability of smart contract code by calling on existing code in external smart contracts. At the same time, code of external smart contracts be executed with the option to appropriately handle errors.  The External-Call Pattern can be applied to handle errors and avoid unintended side effects and increase implementation soundness. Nonetheless, the External-Call Pattern requires additional code to make checks and handle error decreasing code efficiency. 
